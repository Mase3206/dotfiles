...
sp_ln.add_argument(
    "file",
    nargs="*",
    help="(Optional) relative path to file(s) to link. If not given, all files will be linked.",
    default=AVAILABLE_DOTFILES,
)
...
sp_sync.add_argument(
    "file",
    nargs="*",
    help="(Optional) relative path to file(s) to sync. If not given, all files will be synced.",
    default=AVAILABLE_DOTFILES,
)
...
sp_desync = sp_manager.add_parser(
    "desync",
    help="De-sync one or more dotfiles. This converts a once synced dotfile to a local-only dotfile.",
    epilog='NOTE: "relative paths" are relative to the dotfiles directory $DOTFILES_DIR',
)
