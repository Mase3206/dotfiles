...
DOTFILES_MANAGED_FILE = Path(os.environ.get(
    'DOTFILES_MANAGED_FILE',
    DOTFILES_DIR / 'managed.files'
))
if not DOTFILES_MANAGED_FILE.exists():
    raise FileNotFoundError(f"managed.files file (which lists which dotfiles to care about) wasn't found in the dotfiles directory '{DOTFILES_DIR}'")